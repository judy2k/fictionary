...
def main(argv=sys.argv[1:]):
    ...
    try:
        ...
        parser.add_argument(
            '-c', '--count', type=int, default=fictionary.DEFAULT_NUM_WORDS,
            help="The number of words to generate.")
        parser.add_argument(
            '-m', '--min-length', type=int, default=fictionary.DEFAULT_MIN_LENGTH,
            metavar="LENGTH",
        ...
        parser.add_argument(
            '-d', '--dictionary', default=fictionary.DICT_BRITISH_KEY,
        ...
        random_words = fictionary.get_random_words(
            ...
            is_refresh=args.refresh, path=get_cache_filepath()
        )
